...
def appointments_page():
    ...
    # listes pour formulaire de création
...
# --- Création RDV depuis le formulaire (avec chats + employés) ---
...
def appointments_create():
    ...
    db.session.add(appt)
    db.session.flush()  # obtient appt.id sans commit

    # listes multiples
    cats_ids = request.form.getlist("cats[]")
    emp_ids = request.form.getlist("employees[]")

    for cid in cats_ids:
        try:
            cid_int = int(cid)
            if Cat.query.get(cid_int):
                db.session.add(AppointmentCat(appointment_id=appt.id, cat_id=cid_int))
        except ValueError:
            pass
    ...
    for eid in emp_ids:
        try:
            eid_int = int(eid)
            if Employee.query.get(eid_int):
                db.session.add(AppointmentEmployee(appointment_id=appt.id, employee_id=eid_int))
        except ValueError:
            pass
...
def add_vaccination(cat_id):
    ...
    vet = request.form.get("veterinarian") or None
    reaction = request.form.get("reaction") or None
    if not vt_id:
        return redirect(url_for("cat_detail", cat_id=cat_id))
    d = datetime.strptime(date_str, "%Y-%m-%d").date() if date_str else date.today()
    v = Vaccination(cat_id=cat_id, vaccine_type_id=vt_id, date=d, lot=lot, veterinarian=vet, reaction=reaction)
    db.session.add(v)
    db.session.commit()
    return redirect(url_for("cat_detail", cat_id=cat_id))
...
def add_note(cat_id):
    ...
    if content or file_name:
        n = Note(cat_id=cat_id, content=content or None, file_name=file_name)
        db.session.add(n)
        db.session.commit()
    return redirect(url_for("cat_detail", cat_id=cat_id))
...
# --- APIs (inchangées, FullCalendar lit ici) ---
